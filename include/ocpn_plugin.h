...
public:
    ...
    virtual void SetDefaults(); // This will be called upon enabling a PlugIn
                                // via the user Dialog It gives a chance to
                                // setup any default options and behavior
...
public:
    ...
    virtual bool RenderGLOverlayMultiCanvas(
        wxGLContext* pcontext, PlugIn_ViewPort* vp, int canvasIndex);
    virtual bool RenderOverlayMultiCanvas(
...
extern "C" DECL_EXP int InsertPlugInTool(wxString label, wxBitmap* bitmap,
    wxBitmap* bmpRollover, wxItemKind kind, wxString shortHelp,
...
extern "C" DECL_EXP int InsertPlugInToolSVG(wxString label, wxString SVGfile,
    wxString SVGfileRollover, wxString SVGfileToggled, wxItemKind kind,
...
public:
    ...
    virtual wxBitmap& RenderRegionViewOnDCNoText(
        const PlugIn_ViewPort& VPoint, const wxRegion& Region);
    virtual bool RenderRegionViewOnDCTextOnly(
...
extern DECL_EXP wxFont* GetOCPNScaledFont_PlugIn(
    wxString TextElement, int default_size = 0);
...
extern DECL_EXP wxWindow* GetActiveOptionsDialog();
...
extern DECL_EXP wxBitmap GetBitmapFromSVGFile(
...
#define ONLINE_CHECK_RETRY                                                     \
    30 // Recheck the Internet connection availability every ONLINE_CHECK_RETRY
       // s
...
extern DECL_EXP _OCPN_DLStatus OCPN_postDataHttp(const wxString& url,
    const wxString& parameters, wxString& result, int timeout_secs);
...
extern DECL_EXP void PlugInAISDrawGL(
    wxGLCanvas* glcanvas, const PlugIn_ViewPort& vp);
extern DECL_EXP bool PlugInSetFontColor(
    const wxString TextElement, const wxColour color);

// API 1.15
extern DECL_EXP double PlugInGetDisplaySizeMM();

//
extern DECL_EXP wxFont* FindOrCreateFont_PlugIn(int point_size,
    wxFontFamily family, wxFontStyle style, wxFontWeight weight,
...
extern "C" DECL_EXP void CanvasJumpToPosition(
    wxWindow* canvas, double lat, double lon, double scale);
extern "C" DECL_EXP int AddCanvasMenuItem(
    wxMenuItem* pitem, opencpn_plugin* pplugin, const char* name = "");
...
extern "C" DECL_EXP void SetCanvasMenuItemGrey(
    int item, bool grey, const char* name = "");
...
extern DECL_EXP bool CheckMUIEdgePan_PlugIn(
    int x, int y, bool dragging, int margin, int delta, int canvasIndex);
